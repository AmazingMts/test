...
public class HW5Test extends GenericTest {
  ...
  void runTests(Set<String> tests) throws Exception {
    ...
    if (tests.contains("rename")) try {
      ...
      // 创建新表 existingTable
      String existingTable = "nx9T";
      s = openSocket(8001);
      out = new PrintWriter(s.getOutputStream());
      req = "PUT /data/"+existingTable+"/someRow/someCol";
      out.print(req+" HTTP/1.1\r\nHost: localhost\r\nContent-Length: 5\r\n\r\nHello");
      out.flush();
      r = readAndCheckResponse(s, "response");
      if (r.statusCode != 200)
      ...
      // 尝试将 thetable 重命名为 existingTable（已存在）
      s = openSocket(8001);
      out = new PrintWriter(s.getOutputStream());
      req = "PUT /rename/"+thetable;
      out.print(req+" HTTP/1.1\r\nHost: localhost\r\nContent-Length: "+existingTable.length()+"\r\n\r\n"+existingTable);
      out.flush();
      r = readAndCheckResponse(s, "response");
      if (r.statusCode != 409)
        testFailed("The server returned a "+r.statusCode+" response to our attempt to rename a table '"+thetable+"' to '"+existingTable+"', but we were expecting a 409 Conflict. Here is what was in the body:\n\n"+dump(r.body));
      else
        System.out.println("Passed: Server correctly returned 409 Conflict when renaming to an existing table name.");
